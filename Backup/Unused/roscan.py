...
gear_ratios = gear_ratios = [6.75, 75, 75, 24, 33.91, 33.91]  # Replace with your actual gearbox ratios

# Direction inversion for each motor (True/False)
invert_direction = [False, True, True, False, False, False]  # Set True for motors where direction should be inverted
...
def convert_to_can_message(axis_id, speed, position, gear_ratio, invert_direction=False):
    """
    Converts movement parameters into a CAN message for absolute positioning using F5 command.

    :param axis_id: ID of the axis (1-based index)
    :param speed: Movement speed (0-3000 RPM)
    :param position: Absolute target position
    :param gear_ratio: Gear ratio for the specific motor
    :param invert_direction: Whether to invert the movement direction
    :return: CAN message string in hex format
    """
    can_id = format(axis_id, '02X')
    speed_hex = format(speed, '04X')

    # Compute absolute position (absAxis) in pulses
    abs_position = int(position * gear_ratio * 100)

    if invert_direction:
        abs_position = -abs_position

    # Convert abs_position to 24-bit two's complement hex
    abs_position_hex = format(abs_position & 0xFFFFFF, '06X')

    # Construct the CAN message with F5 command
    can_message = can_id + 'F5' + speed_hex + '02' + abs_position_hex
